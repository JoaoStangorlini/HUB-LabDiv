...
export default function AutoresPage() {
    ...
    const handleReject = async (id: string, currentStatus: string) => {
        if (currentStatus === 'rejeitado') return;
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            default:
                return <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded text-[10px] font-bold uppercase tracking-wider">Pendente</span>;
        }
    };
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="flex flex-col gap-1">
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Buscar por Autor</h1>
                    <p className="text-slate-500 dark:text-slate-400">Encontre todas as submissões enviadas por um determinado pesquisador ou aluno.</p>
                </div>
            </div>
            ...
            {hasSearched && (
                <div className="flex flex-col gap-4 mt-2">
                    <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">
                        Resultados encontrados: {submissions.length}
                    </h2>

                    {isLoading ? (
                        <div className="text-center py-10 text-slate-500">Buscando no banco de dados...</div>
                        ...
                        <div className="text-center py-12 bg-white dark:bg-[#1e293b] rounded-xl border border-slate-200 dark:border-slate-700">
                            ...
                            <h3 className="text-lg font-medium text-slate-500 dark:text-slate-400">Nenhuma submissão encontrada para este autor.</h3>
                        </div>
                    ) : (
